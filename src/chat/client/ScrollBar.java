...
public class ScrollBar extends Canvas implements Runnable {
    ...
    Graphics Graph;
    Image Img;
    boolean Down, Down1, Down2;
    int Dwidth, Dheight, LastValue, LastX, LastY, BsizeX, BsizeY, Width, Height, Type;
    public boolean Disabled;
    public int ScrollValue,TotalArea, ViewArea, PageValue, LineValue;
    boolean running;
    Thread runner;
    Event lastMouseDown;
    int sleeptime;
    ...
    public boolean mouseDown(Event event, int i, int j) {
        ...
        if (Type == 1) {
            if (i >= 0 && i <= BsizeX) {
                doLineUp();
                start(event);
            } else if (i >= size().width - BsizeX && i <= size().width) {
                doLineDown();
                start(event);
            } else if (i < Scroller.x) {
                doPageUp();
                start(event);
            } else if (i > Scroller.x + Scroller.width) {
                doPageDown();
                start(event);
            }
        } else if (j >= 0 && j <= BsizeY) {
            doLineUp();
            start(event);
        } else if (j >= size().height - BsizeY && j <= size().height) {
            doLineDown();
            start(event);
        } else if (j < Scroller.y) {
            doPageUp();
            start(event);
        } else if (j > Scroller.y + Scroller.height) {
            doPageDown();
            start(event);
        }
        return true;
